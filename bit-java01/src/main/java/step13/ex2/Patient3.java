...
public class Patient3 {
  ...
  void setAge(int age) {
    if (age < 1 || age > 150) // 값이 유효하지 않으면 호출자에게 알린다.
      throw new RuntimeException("age 값이 유효하지 않습니다."); 
    this.age = age;
  }
  int getAge() {
    return this.age;
  ...
  void setWeight(float weight) {
    if (weight < 1 || weight > 500f)
      throw new RuntimeException("weight 값이 유효하지 않습니다."); 
    this.weight = weight;
  }
  float getWeight() {
    return this.weight;
  ...
  void setHeight(float height) {
    if (height < 1 || height > 300f)
      throw new RuntimeException("height 값이 유효하지 않습니다."); 
    this.height = height;
  }
  float getHeight() {
    return this.height;
